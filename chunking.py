...
FRONTMATTER_RE = re.compile(r"^---\n(.*?)\n---\n(.*)$", re.DOTALL) #regex to check if the files meta data is provided in top of the file as yaml format between ---
HEADING_RE = re.compile(r"^##\s+(.+)$", re.MULTILINE) #regex to identify the header text in md file
...
def split_by_heading(body: str) -> list[tuple[str, str]]:
    """Return [(heading, section_text), ...], dropping the leading H1 title line."""
